...
namespace osu.Game.Rulesets.Katsudon.Skinning.Argon
{
    internal partial class ArgonFlourishTriggerSource : DrumSampleTriggerSource
    {
        ...
        public override void Play(Taiko.Objects.HitType hitType, bool strong)
        {
            KatsudonHitObject? hitObject = GetMostValidObject() as KatsudonHitObject;

            if (hitObject == null)
                return;

            var originalSample = hitObject.CreateHitSampleInfo(hitType == Taiko.Objects.HitType.Rim ? HitSampleInfo.HIT_CLAP : HitSampleInfo.HIT_NORMAL);
            ...
            if (strong && hitType == Taiko.Objects.HitType.Rim && canPlayFlourish(hitObject))
                PlaySamples(new ISampleInfo[] { new VolumeAwareHitSampleInfo(hitObject.CreateHitSampleInfo(HitSampleInfo.HIT_FLOURISH), true) });
        }

        private bool canPlayFlourish(KatsudonHitObject hitObject)
        {
            ...
            var hitObjects = hitObjectContainer.AliveObjects
                                               ...
                                               .OfType<Hit>()
                                               .Where(h => h.IsStrong && h.Type == Taiko.Objects.HitType.Rim);
